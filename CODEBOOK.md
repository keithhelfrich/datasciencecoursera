...
``` 
A brief description of each data file is provided below:

- `features_info.txt`: Information about the variables used on the feature vector
- `features.txt`: List of feature names
- `activity_labels.txt`: Links activity class labels with activity name
- `train/X_train.txt`: Training set of 561 features, features are normalized & bound within [-1,1]
...
- `train/subject_train.txt`: One row per subject for each window sample, range is from 1 to 30
- `test/subject_test.txt`: One row per subject for each window sample, range is from 1 to 30.


#### <ins>Processing</ins>

A detailed description is found in `README.md`. Find below additional re: the data processing

   1. "### Loading Packages       ###" - loads reshape2 package in R
      - the reshape2 package is required for melting and decasting data frames
   2. "### Reading Dirty Datasets ###" - reads original data sets
      "### Please Be Patient      ###" - this can take some time
      - original dirty data is read into memory
      - for each of X, Y and s: the test & train data are bound together into one
      - as needed, column names are defined
      - featureNames are loaded into memory, as well
   3. "### Subsetting             ###" - subsets original data to only those features which contain 'mean' or 'std'
      - features of interest are identified as those which contain `mean()` or `std()`
      - featureData is subset to discard all features except for those of interest
      - featureData is given accurate column names
   4. "### Formatting             ###" - assigns pretty feature names and pretty activity names
      - beautification occurs
      - pretty feature names and pretty activity names are assigned
      - text manipulation is performed to add spacing & convert names to lower case
   5. "### Merging                ###" - performs translation from activityID to activityName
      - activityLabels & activityNames are merged into one
      - activityName is converted into a factor variable
   6. "### Binding                ###" - creates a clean & tidy data frame
      - the various components are bound together by column into a single data frame: subjectLabels, activityData$activityName, and featureData
      - a cleanup is made to the name for the column activityName
   7. "### Writing                ###" - writes clean_data.csv & clean_data.txt
      - the first data set is written to disk in two formats: csv and tab delimited
   8. "### Melting                ###" - melts the clean & tidy data frame with subjectID & activityName as IDs and all other fields as Measures
      - subjectID and activityName are declared as IDs: these are the identifiers for later decasting to compute the average of the remaining Measures
  ...
  10. "### Writing                ###" - writes the decast data as tidy_data.csv and tidy_data.txt
      ...
      - each of the mean() and std() features aggregated as an average
  11. "### All Done!              ###" - all done!


#### <ins>Tidy Dataset</ins>

A detailed description of the tidy data set variables:
